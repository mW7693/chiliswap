...
const ChiliBar = artifacts.require('ChiliBar');

contract('ChiliBar', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.chili = await ChiliToken.new({ from: alice });
        this.bar = await ChiliBar.new(this.chili.address, { from: alice });
        this.chili.mint(alice, '100', { from: alice });
        this.chili.mint(bob, '100', { from: alice });
        this.chili.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.chili.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.chili.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.chili.approve(this.bar.address, '100', { from: alice });
        await this.chili.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.chili.balanceOf(this.bar.address)).valueOf(), '30');
        // ChiliBar get 20 more SUSHIs from an external source.
        await this.chili.transfer(this.bar.address, '20', { from: carol });
        ...
        assert.equal((await this.chili.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.chili.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.chili.balanceOf(bob)).valueOf(), '98');
    });
});
