...
const ChiliMaker = artifacts.require('ChiliMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('ChiliMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.chili = await ChiliToken.new({ from: alice });
        await this.chili.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await ChiliMaker.new(this.factory.address, bar, this.chili.address, this.weth.address);
        this.chiliWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.chili.address)).logs[0].args.pair);
    ...
    it('should make SUSHIs successfully', async () => {
        ...
        await this.weth.transfer(this.chiliWETH.address, '10000000', { from: minter });
        await this.chili.transfer(this.chiliWETH.address, '10000000', { from: minter });
        await this.chiliWETH.mint(minter);
        ...
        assert.equal((await this.chili.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for SUSHI-ETH pair
        await this.chili.transfer(this.chiliWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.chiliWETH.address, '100000', { from: minter });
        await this.chiliWETH.sync();
        await this.chili.transfer(this.chiliWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.chiliWETH.address, '10000000', { from: minter });
        await this.chiliWETH.mint(minter);
        assert.equal((await this.chiliWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.chili.address, this.weth.address);
        assert.equal((await this.chili.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.chiliWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});